...
\____/_.___/\__,_/_/ /_/\__/\__,_/   /____/_____(_)____/  /_/       |___/_/  /_/ (Ubuntu 25.10 - Questing Quokka)
...
function advanced_settings() {
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Ubuntu 25.10 VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a Ubuntu 25.10 VM using the above advanced settings${CL}"
  else
    header_info
...
msg_info "Retrieving the URL for the Ubuntu 25.10 Disk Image"
...
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Ubuntu 25.10 VM"
  qm start $VMID
  msg_ok "Started Ubuntu 25.10 VM"
fi
